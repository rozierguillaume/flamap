...
export function renderList(container, fires, onSelect) {
  ...
  if (!fires.length) {
    container.append(Object.assign(document.createElement('p'), {
      className: 'archive-status', textContent: 'Aucun feu archivé pour le moment.',
    }));
    return;
  }
  for (const fire of fires) {
    ...
    title.textContent = fire.commune || 'Commune inconnue';
    ...
    badge.textContent = fire.status || 'Statut inconnu';
