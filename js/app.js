...
function applyBurnt() {
  ...
  if (map.getLayer('nrt-fill')) map.setPaintProperty('nrt-fill', 'fill-opacity', .30 * k);
...
function setExportMapTime(exportMap, ts) {
  ...
  for (const id of ['hotspots-overview', 'hotspots'])
    if (exportMap.getLayer(id)) exportMap.setPaintProperty(id, 'circle-color', color);

  const land = rampAt(AGE_SIZE, APPEAR);
  const radiiAt = k => ['interpolate', ['linear'], age,
    0, .25 * land * k * EXPORT_UI_SCALE,
    APPEAR * .45, 1.18 * land * k * EXPORT_UI_SCALE,
    APPEAR, land * k * EXPORT_UI_SCALE,
    ...rampAfter(AGE_SIZE, APPEAR, k * EXPORT_UI_SCALE)];
  const detailRadius = ['interpolate', ['linear'], ['zoom'],
    ...ZOOM_SCALE.flatMap(([z, k]) => [z, ['*', radiiAt(k), FRP_DETAIL_SIZE]])];
  if (exportMap.getLayer('hotspots'))
    exportMap.setPaintProperty('hotspots', 'circle-radius', detailRadius);
  if (exportMap.getLayer('hotspots-overview')) {
    const countScale = ['interpolate', ['linear'], ['get', 'n'],
      1, 1, 10, 1.7, 100, 3.1, 1000, 5.2];
    exportMap.setPaintProperty('hotspots-overview', 'circle-radius',
  ...
  if (exportMap.getLayer('nrt-fill'))
    exportMap.setPaintProperty('nrt-fill', 'fill-opacity', .30 * k);
  for (const id of ['recent-line', 'burnt-line'])
    if (exportMap.getLayer(id)) exportMap.setPaintProperty(id, 'line-opacity', .95 * k);
  if (exportMap.getLayer('nrt-line'))
    exportMap.setPaintProperty('nrt-line', 'line-opacity', .55 * k);
...
async function init() {
  ...
  map.addLayer({ id: 'nrt-fill', type: 'fill', source: 'nrt',
    ...
    paint: { 'fill-color': V('--nrt'), 'fill-opacity': .30,
  ...
  map.addLayer({ id: 'nrt-line', type: 'line', source: 'nrt',
    minzoom: manifest.legacy ? 0 : manifest.detail_zoom,
    paint: { 'line-color': V('--nrt-edge'), 'line-width': 1,
